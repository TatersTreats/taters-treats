const PRODUCTS = [
  ...
  {const PRODUCTS = [
  {
    ...
    defaultSize: "regular",
    sizes: {
      trial:   { label: "Trial",  priceId: "price_1TD3rlDywMn3O3R8C2mIqpVZ" },
      regular: { label: "Regular", priceId: "price_1TD3rlDywMn3O3R8psJph7ti" },
      value:   { label: "Value",  priceId: "price_1TD3rlDywMn3O3R8fHQICEqm" }
    }
  },
  {
    id: "pbmc",
    ...
    defaultSize: "regular",
    sizes: {
      trial:   { label: "Trial",  priceId: "price_1TD3rlDywMn3O3R84BZEFkVl" },
      regular: { label: "Regular", priceId: "price_1TD3rlDywMn3O3R8Kw2mxifP" },
      value:   { label: "Value",  priceId: "price_1TD3rkDywMn3O3R8LhNyxt0V" }
    }
  },
  {
    id: "ginger",
    ...
    defaultSize: "regular",
    sizes: {
      trial:   { label: "Trial",  priceId: "price_1TD3rmDywMn3O3R81CgJYyr4" },
      regular: { label: "Regular", priceId: "price_1TD3rlDywMn3O3R8CDw2xmaI" },
      value:   { label: "Value",  priceId: "price_1TD3rkDywMn3O3R8PquAjDEM" }
    }
  }
];
...
function openProductDetail(card) {
  ...
  overlay.setAttribute("aria-label", "Close product details");
...
function attachProductCardHandlers() {
  document.querySelectorAll(".product-card").forEach((card) => {
    card.addEventListener("click", (event) => {
      if (event.target.closest(".pill-btn, .qty-button, .add-button")) return;
      openProductDetail(card);
    });
  });
}

function addToBowl(productId, options = {}) {
  const product = getProduct(productId);
  if (!product) return;

  const sizeKey = options.sizeKey || getSelectedSizeKey(productId);
  const quantity = options.quantity || getSelectedQuantity(productId);
...
function renderProductCard(product) {
  ...
  card.innerHTML = `
    ...
    <div class="product-body">
      ...
      <p class="product-benefit">${product.description || product.benefit}</p>
...
document.addEventListener("click", (event) => {
  ...
  if (sizeButton) {
    const { product: productId } = sizeButton.dataset;
